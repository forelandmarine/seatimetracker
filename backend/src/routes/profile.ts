        ...
        response: {
          200: {
            ...
            properties: {
              ...
              testSubscriptionActive: { type: 'boolean' },
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        app.logger.error({ userId: user.id, err: error }, 'Error reading multipart file');
        return reply.code(400).send({ error: 'Failed to read file from request' });
      }
      ...
      try {
        ...
        app.logger.warn({ userId: user.id, err: error }, 'File size exceeded or error reading buffer for profile image');
      ...
      const imageKey = `profile-images/${user.id}/${Date.now()}-${filename}`;
      let uploadedKey: string;
      try {
        uploadedKey = await app.storage.upload(imageKey, buffer);
        app.logger.info({ userId: user.id, imageKey: uploadedKey, filename }, 'Profile image uploaded successfully');
      } catch (error) {
        app.logger.error({ userId: user.id, imageKey, filename, err: error }, 'Failed to upload profile image to storage');
