...
export function register(app: App, fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { fleet_id: string } }>('/api/tracking/fleet/:fleet_id', {
    ...
    // Validate fleet_id format (should start with 'fleet_')
    if (!fleet_id.startsWith('fleet_')) {
      app.logger.warn({ fleet_id }, 'Invalid fleet ID format');
      return reply.code(404).send({ error: 'Fleet not found' });
    }
